...
module.exports = (bot, {
  ...
  if (process.env.PRISMARINE_VIEWER_CAMERA_FORWARD === 'true') {
    viewer.camera.rotation.order = 'YXZ'
  }
  let cameraLogCount = 0

  if (!viewer.setVersion(bot.version)) {
    return false
  }
  const initialYaw = Number.isFinite(fixedYaw) ? fixedYaw : (bot.entity.yaw + yawOffset)
  const initialPitch = Number.isFinite(fixedPitch) ? fixedPitch : (bot.entity.pitch + pitchOffset)
  if (process.env.PRISMARINE_VIEWER_CAMERA_FORWARD === 'true') {
    viewer.camera.position.set(bot.entity.position.x, bot.entity.position.y + viewer.playerHeight, bot.entity.position.z)
    viewer.camera.rotation.set(initialPitch, initialYaw, 0)
  } else {
    viewer.setFirstPersonCamera(bot.entity.position, initialYaw, initialPitch)
  }
  ...
  function botPosition () {
    const yaw = Number.isFinite(fixedYaw) ? fixedYaw : (bot.entity.yaw + yawOffset)
    const pitch = Number.isFinite(fixedPitch) ? fixedPitch : (bot.entity.pitch + pitchOffset)
    if (process.env.PRISMARINE_VIEWER_CAMERA_FORWARD === 'true') {
      viewer.camera.position.set(bot.entity.position.x, bot.entity.position.y + viewer.playerHeight, bot.entity.position.z)
      viewer.camera.rotation.set(pitch, yaw, 0)
    } else {
      viewer.setFirstPersonCamera(bot.entity.position, yaw, pitch)
    }
  ...
  function update () {
    ...
    if (cameraLogCount < 5) {
      ...
      console.log(`[prismarine-viewer] camera pos=(${cam.x.toFixed(2)}, ${cam.y.toFixed(2)}, ${cam.z.toFixed(2)}) rot=(${rot.x.toFixed(2)}, ${rot.y.toFixed(2)}, ${rot.z.toFixed(2)}) bot pos=${botPos ? botPos.toString() : 'unknown'}`)
    }
